...
describe('Token', () => {
  ...
  describe('#constructor', () => {
    it('fails with invalid address', () => {
      expect(() => new Token(5, '0xhello00000000000000000000000000000000002', 18).address).toThrow(
        '0xhello00000000000000000000000000000000002 is not a valid address'
      )
    })
    it('fails with negative decimals', () => {
      expect(() => new Token(5, ADDRESS_ONE, -1).address).toThrow('DECIMALS')
    })
    it('fails with 256 decimals', () => {
      expect(() => new Token(5, ADDRESS_ONE, 256).address).toThrow('DECIMALS')
    })
    it('fails with non-integer decimals', () => {
      expect(() => new Token(5, ADDRESS_ONE, 1.5).address).toThrow('DECIMALS')
    })
  })
  ...
  describe('#equals', () => {
    ...
    it('false if chain id differs', () => {
      expect(new Token(5, ADDRESS_ONE, 18).equals(new Token(1, ADDRESS_ONE, 18))).toBe(false)
    })
